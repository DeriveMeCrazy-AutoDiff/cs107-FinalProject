...
class ReverseAutoDiff():
    ...
    def __init__(self, value, variables = 1, position = 0):
        self.val = value
        self.children = []
    ...
    def __add__(self, other):
        try:
            ...
            node.children.append((0.0, other_node))
            node.children.append((1.0, self))
        node.op = 'add'
        return node
    ...
    def __mul__(self, other):
        try:
            node = ReverseAutoDiff(self.val*other.val)
            node.children.append((other.val, self))
            node.children.append((self.val, other))
        except AttributeError:
            node = ReverseAutoDiff(self.val*other)
            other_node = ReverseAutoDiff(other)
            node.children.append((other_node.val, self))
            node.children.append((0.0, other_node))
        node.op = 'mul'
        return node
